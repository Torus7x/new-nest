...
export class UfdService{
    ...
    async getFabricVersionList() {
        const res = await this.appService.getJsonData("UF:defaultJson");
        return Object.keys(JSON.parse(res));
    }

    async getFabricVersion(version: any) {
        const response = await this.appService.getJsonData("UF:defaultJson");
    ...
    async updateFabric(req: any, version: any) {
        ...
        if (versionList.includes(version)) {
            ...
            const res = await this.appService.setJsonData(key,data);
            return res;
        } else {
            return "No FabricVersion Found"
        }
